        ...
        for obj_num in range(0, length_prim_block):
            ...
            for actual_prim_number in range(0, current_number_primitives):
                ...
                # 10 Rows
                    ...
                    print(f'New Primitive Type Found: {cur_prim_block[0:4]}, \nInterrupting Conversion to avoid future errors...')
                    print(f'This is found in the Object Number: {obj_num},\nPrimitive Number: {actual_prim_number}')
                    exit()

            """Reading the bits to know which is the Primitive Type have to be efficient, so in order to make that
            i will taking care of the next two important values in the Primitive Packet header.
            FLAG and MODE

            # FLAG 0-0-0-0-0-GRD-FCE-LGT
            # GRD |||| 0 = Single Coloured Poly ; 1 = Gradation Poly
            # FCE |||| 0 = Single Faced Poly ; 1 = Double Faced Poly :::: NOTE: Only valid if the Primitive is a Polygon :::: NOTE 2: ACTUALLY I DON'T CARE BECAUSE THIS IS IGNORED
            # LGT |||| 0 = Light Source Calculation ON ; 1 = Light Source Calculation OFF

            # MODE CODE-OPTION (3 Bits - 5 Bits)
            # CODE |||| 0-0-1 = Polygon (Tri or Quad) ; 0-1-0 = Straight Line ; 0-1-1 = Sprite :::: NOTE: Straight Lines and Sprites are ignored in this version of the Tool
            # OPTION == IIP - [0/1] - TME - ABE - TGE
            # IIP = Shading Mode |||| 0 = Flat Shading ; 1 = Gouraud Shading
            # [0/1] = Poly Number |||| 0 = 3 Vertex ; 1 = 4 Vertex
            # TME = Texture Specification |||| 0 = OFF ; 1 = ON (If texture or not)
            # ABE = Translucency Processing |||| 0 = OFF ; 1 = ON (If the Poly is Translucent or Solid)
            # TGE = Brightness Calculation in Texture Mapping |||| 0 = ON ; 1 = OFF (If OFF == True, Draw Texture as is)"""
            ...
            for primitive_to_decode in primitives_split:
                """length_prim_packet = len(primitive_to_decode) # I WILL MAINTAIN THIS IF NEEDED
                primitive_mode = primitive_to_decode[3:4] # GETTING THE MODE BYTE
                primitive_flag = primitive_to_decode[2:3] # GETTING THE FLAG BYTE
                print(primitive_mode, primitive_flag)"""
                # 3 Vertex LSC
